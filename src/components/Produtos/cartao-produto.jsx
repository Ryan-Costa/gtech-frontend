...
const CartaoProduto = ({ id, nome, categoria, desconto, preco, imagem }) => {
  ...
  return (
    <Link to={`/product/${id}`} className="w-full flex flex-col gap-4 rounded">
      <div className="h-[321px] w-full bg-white relative flex items-center justify-center">
        <span className="bg-[#E7FF86] absolute left-5 top-5 py-1 px-3 rounded-full text-[#474747] font-bold text-sm">
          {desconto * 100}% OFF
        </span>
      ...
      <div className="flex flex-col">
        ...
        <p className="text-2xl space-x-1.5">
          <span className="font-normal line-through text-[#8F8F8F]">
            {precoOriginal}
          </span>
